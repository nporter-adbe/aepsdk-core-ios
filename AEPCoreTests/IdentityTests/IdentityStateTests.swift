...
class IdentityStateTests: XCTestCase {
    ...
    func testProcessPrivacyChangeToOptIn() {
        ...
        state = IdentityState(identityProperties: props, hitQueue: MockHitQueue(processor: MockHitProcessor()))
        let event = Event(name: "Test event", type: .identity, source: .requestIdentity, data: [ConfigurationConstants.Keys.GLOBAL_CONFIG_PRIVACY: PrivacyStatus.optedIn])
    ...
    func testProcessPrivacyChangeToOptOut() {
        ...
        state = IdentityState(identityProperties: props, hitQueue: MockHitQueue(processor: MockHitProcessor()))
        let event = Event(name: "Test event", type: .identity, source: .requestIdentity, data: [ConfigurationConstants.Keys.GLOBAL_CONFIG_PRIVACY: PrivacyStatus.optedOut])
    ...
    func testProcessPrivacyChangeFromOptOutToUnknown() {
        ...
        state = IdentityState(identityProperties: props, hitQueue: MockHitQueue(processor: MockHitProcessor()))
        let event = Event(name: "Test event", type: .identity, source: .requestIdentity, data: [ConfigurationConstants.Keys.GLOBAL_CONFIG_PRIVACY: PrivacyStatus.unknown])
