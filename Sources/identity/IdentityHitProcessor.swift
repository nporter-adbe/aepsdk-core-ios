...
class IdentityHitProcessor: HitProcessable {
    ...
    private func handleNetworkResponse(connection: HttpConnection, completion: @escaping (Bool) -> ()) {
        if connection.responseCode == 200 {
            // hit sent successfully
            guard let data = connection.data, let identityResponseObject = try? JSONDecoder().decode(IdentityRequestResponse.self, from: data) else {
